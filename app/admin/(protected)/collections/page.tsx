...
export default function AdminCollectionsPage() {
  ...
  const load = () => {
    fetch("/api/collections")
      ...
      .then((data) => {
        if (!data.collections || data.collections.length === 0) throw new Error();
  ...
  return (
    <div>
      <div className="flex items-center justify-between">
        <div>
          <h1 className="font-display text-3xl uppercase">Catégories</h1>
          <p className="mt-1 font-mono text-[11px] uppercase tracking-widest2 text-ash">
            {collections.length} catégorie(s) {usingMock && "· données de démo"}
          </p>
        </div>
        <Link
      ...
      <div className="mt-8 grid grid-cols-1 gap-4 sm:grid-cols-2">
        {collections.map((c) => (
          <div key={c.slug} className="border hairline p-5">
            <div className="flex items-start justify-between">
              <div>
                <p className="font-display text-xl uppercase">{c.name}</p>
                <p className="mt-1 font-mono text-[11px] text-signal">{c.tagline}</p>
              </div>
              <div className="flex gap-3">
                <Link href={`/admin/collections/${c.slug}`} aria-label="Modifier">
                  <Pencil className="h-4 w-4 text-ash hover:text-bone" />
                </Link>
                <button onClick={() => handleDelete(c.slug)} aria-label="Supprimer">
                  <Trash2 className="h-4 w-4 text-ash hover:text-signal" />
                </button>
              </div>
            </div>
            <p className="mt-3 text-sm text-ash">{c.description}</p>
          </div>
        ))}
      </div>
    </div>
